...
def calculate_totals(df5: DataFrame) -> DataFrame:
    return (df5.withColumn("sub_total",F.coalesce(F.col("English"),F.lit(0))
                         + F.coalesce(F.col("Hindi"),F.lit(0)) + 
                         F.coalesce(F.col("Telugu"),F.lit(0)) +
                         F.coalesce(F.col("Maths"),F.lit(0)) +
                         F.coalesce(F.col("Science"),F.lit(0)) +
                         F.coalesce(F.col("Social"),F.lit(0))).\
                        select('academic_year', 'school', 'student_name', 
               'exam_name','sub_total'))
